const BASE_URL = `https://khaos-api-sit-run-2.tkg-qa.spdigital.io/khaos/v1/halp`;
...
async function getSchedulesByDate(currentDate) {
    const url = new URL(`${BASE_URL}/available-schedules`);
...
async function countCallInQueue() {
    const res = await fetch(`${BASE_URL}/sessions/count-queued`);
    if (!res.ok) throw new Error("Failed to fetch queue count");
    return res.json();
}

function subscribeAgentJoinedEvent(sessionId) {
    return new EventSource(`${BASE_URL}/sessions/agent-joined?sessionId=${sessionId}`);
}
...
async function rejoin(sessionId) {
    const res = await fetch(`${BASE_URL}/${sessionId}/rejoin`, {
        method: "POST",
        headers: getAuthHeader(),
    });
...
async function changeStatus(sessionId, status) {
    const url = new URL(`${BASE_URL}/${sessionId}`);
...
async function updateTxnNature(request) {
    const res = await fetch(`${BASE_URL}/transactionNature`, {
        method: "POST",
        headers: {
            ...getAuthHeader(),
